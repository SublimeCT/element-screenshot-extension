...
- 区域截图(点选元素后只使用从该元素自身开始向上的第一个真实可滚动元素；不可滚动节点本身不得成为区域截图目标)
...
- 整页截图(自动滚动页面并拼接)
- 手动滚动截图:选择页面或可滚动元素区域；点选不可滚动节点时自动改为该节点的第一个可滚动祖先,不可滚动节点本身不得成为截图或滚动监听目标。确认后先截取初始帧；用户手动滚动目标,每次滚动停止后追加一帧,按任意键结束并按实际滚动距离裁剪重叠后拼接
...
- 区域模式只保留最后一个目标并持续显示锁定高亮,点选节点自身可滚动时锁定自身,否则必须锁定祖先链中距离该节点最近的可滚动元素；如果该祖先是页面根滚动目标则调用 `capturePage()`,否则调用 `captureElement()`,普通不可滚动节点本身不得成为区域截图目标。手动滚动模式优先默认可滚动的页面根元素,根元素不可滚动时退回第一个可滚动下级元素,不存在任何可滚动目标时不高亮元素并禁用截图确认。手动模式允许重新选择目标,点选节点自身可滚动时锁定自身,否则锁定祖先链中距离该节点最近的可滚动元素,普通不可滚动节点本身不得成为目标；整页模式默认高亮页面根元素,根元素不可滚动时退回第一个可滚动下级元素。选中页面根元素时调用 `capturePage()`,选中下级元素时调用 `captureElement()`
...
当 `scrollTop` 到达 `scrollHeight - clientHeight`(即最后一屏)时,若上一步的滚动距离不是 `clientHeight` 的整数倍,最后一屏与倒数第二屏会有重叠区域。拼接时需要按**实际滚动距离**裁剪每一屏的高度,而不是固定用 `clientHeight`,确保不出现重复或空白间隙。最终 PNG 的物理像素高度以所有已裁剪帧的整数物理像素高度之和为唯一事实来源,不得再用 CSS 累计高度换算后的独立舍入值否决已经取得的有效帧。
...
- 手动滚动模式的监听目标必须是页面根滚动目标或 `overflow-y:auto/scroll` 且 `scrollHeight > clientHeight` 的元素；点选普通子节点时沿祖先链向上查找并锁定第一个满足条件的元素,禁止监听不会产生 `scroll` 事件的普通节点；如果页面中不存在任何有效滚动目标,保持选择状态并禁用截图确认
