...
本插件的目标:**不修改页面任何样式**,通过"滚动目标元素本身 + 多次截取可视区域真实像素 + 按元素矩形裁剪 + 拼接"的方式,得到目标元素的完整内容截图(含被自身滚动条隐藏的部分)。
...
6. 截图过程中始终**不修改页面任何 DOM/CSS**,只操作目标元素自身的 `scrollTop`(截图完成后立即还原到原始滚动位置)。
...
│   ├── content.ts           # content script 入口:元素选取交互 + 滚动控制 + 触发截图请求
│   ├── content.css          # 选取模式下的高亮遮罩样式(WXT 会自动关联同名 content script)
│   └── popup/
...
│   ├── capture.ts            # 滚动 + 截图循环的核心逻辑(被 content.ts 调用)
...
├── package.json
├── tsconfig.json
├── test-pages/                 # 手动测试用的本地 HTML 页面(与 AGENTS.md §5 测试策略呼应)
└── .gitignore                  # 需排除 WXT 构建产物目录 .output/、.wxt/
```

> 说明:WXT 默认构建产物在 `.output/`(打包后的可加载扩展 + 提交商店用的 zip 均在此目录下生成),开发调试临时文件在 `.wxt/`,两者都不进 Git。

### 4.3 各模块职责

**entrypoints/background.ts(service worker)**
- 监听来自 content script 的 `CAPTURE_TAB` 消息,调用 `chrome.tabs.captureVisibleTab`,返回 dataURL
- 监听 `chrome.action.onClicked`(如果不使用 popup,直接用这个触发选取模式),向当前标签页发送 `START_PICK` 消息
...
```
用户点击图标
  → entrypoints/background.ts: chrome.action.onClicked
  → 发送 START_PICK 到 content script
  → entrypoints/content.ts 进入选取模式(高亮 + 监听 click)
  → 用户点击目标元素
...
- 每次设置 `scrollTop` 后,不能立即截图,需等待:
  ...
  2. 一个可配置的短暂延时(默认 80–150ms,兼容懒加载图片/过渡动画),该值可作为设置项暴露给用户调整
...
- MVP 阶段**不支持**跨 iframe 边界的元素选取(content script 默认只注入到顶层 frame;如需支持 iframe 内元素,需要 `"all_frames": true` 并处理跨 frame 消息通信,列入后续迭代)
...
- `scripting`:用于向当前标签页按需注入/执行脚本(WXT 的 content script 默认通过 `entrypoints/content.ts` 静态声明匹配规则运行;如需按需注入而非静态声明,才需要显式用到 `scripting` API)
...
- [ ] 截图过程不修改任何页面 DOM 结构或 CSS(可临时切换 `visibility`,但必须在结束时百分之百还原)
